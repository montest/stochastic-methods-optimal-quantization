...
def apply_M_gradient_descend_steps(centroids: List[Point], xs: List[Point], probas: List[float], distortion: float, init_n: int, use_optimized_version: bool):
...
def clvq_method(N: int, n: int, nbr_iter: int, use_optimized_version: bool = True):
    ...
    with trange(nbr_iter, desc='CLVQ method') as t:
        for step in t:
            ...
            centroids, probas, distortion = apply_M_gradient_descend_steps(centroids, xs, probas, distortion, init_n=step*M, use_optimized_version=use_optimized_version)
